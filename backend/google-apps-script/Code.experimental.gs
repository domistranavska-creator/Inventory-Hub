...
function syncItemsFromKros_() {
  ...
  try {
    ...
    Logger.log("KROS experimental sync finished with %s items.", items.length);
    return items.length;
  } finally {
    lock.releaseLock();
  }
...
function fetchAllKrosCatalogItems_(token) {
  ...
  while (true) {
    ...
    page += 1;
    Logger.log("KROS experimental page %s skip=%s batch=%s", page, skip, batch.length);
...
function mapKrosCatalogItem_(item) {
  ...
  return {
    ...
    stockQty: toPositiveInt_(ltQty + prQty),
